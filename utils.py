...
def add_image_focus(image, focus):
    ...
    focus_array = [ -0.5 for i in range(len(image[0]))]
    for focus_index in focus:
        focus_array[focus_index] = 0.5

    image = np.append(image, [focus_array], axis = 0)

    # print("after", image.shape)
    return image

def create_input_files_with_focus(dataset, karpathy_json_path, image_folder, captions_per_image, min_word_freq, output_folder,
    ...
    for img in data['images']:
        ...
        if img['split'] in {'train', 'restval'}:
            for i, caption in enumerate(captions):
                ...
                train_image_focus.append([focuses[i]])
            
        elif img['split'] in {'val'}:
            for i, caption in enumerate(captions):
                val_image_paths.append(path)
                val_image_captions.append([caption])
                val_image_focus.append([focuses[i]])

        elif img['split'] in {'test'}:
            for i, caption in enumerate(captions):
                test_image_paths.append(path)
                test_image_captions.append([caption])
                test_image_focus.append([focuses[i]])
    ...
    seed(123)
    for impaths, imcaps, imfocus, split in [(train_image_paths, train_image_captions, train_image_focus, 'TRAIN'),
                                   (val_image_paths, val_image_captions, val_image_focus, 'VAL'),
                                   (test_image_paths, test_image_captions, test_image_focus, 'TEST')]:
        print("image hdf5 route", os.path.join(output_folder, split + '_IMAGES_' + base_filename + '.hdf5'))
        ...
        with h5py.File(os.path.join(output_folder, split + '_IMAGES_' + base_filename + '.hdf5'), 'a') as h:
            ...
            if image_type == "pixel":
                ...
                images = h.create_dataset('images', (len(impaths), svg_channel + 1, max_element_number), dtype='float32')
                # 维度增加1，表示关注。
            ...
            for i, path in enumerate(tqdm(impaths)):
                ...
                img = add_image_focus(img, imfocus[i][0])
...
def create_input_files_replace_token(dataset, karpathy_json_path, image_folder, captions_per_image, min_word_freq, output_folder,
                       max_len=100, image_type = "pixel", need_text = False, max_element_number = 40, focus = False):
            ...
            for i, path in enumerate(tqdm(impaths)):
                ...
                for j, caption in enumerate(captions):
                    ...
                    for word in replace_token:
                        caption = caption.replace(" " + word + " ", " " + replace_token[word] + " ")
                    caption = caption.strip()
                    current_captions = caption.split(";")
                    current_captions = [c.strip().split(" ") for c in current_captions]
                    new_captions.extend(current_captions)
                    # captions[j] = caption.split(" ")
...
def save_checkpoint(data_name, epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer, decoder_optimizer,
    ...
    filename = '{}/epoch_{:0>3d}_bleu_{:.4f}.pth.tar'.format(checkpoint_path, epoch, bleu4)
